...
The **Sidebar Window** is the broad compatibility path for plugins that already integrate with Gutenberg's Post / Block sidebar system. It is not a per-plugin shim: the source editor's outer title-bar button opens a launch chooser for Post, Block, and plugin areas while leaving the source's current complementary area open. Gutenberg's store exposes the active identifier but not an enumeration of every registered area, so OpenStation discovers plugin registrations from the same `aria-controls` identifiers used by Gutenberg's pinned and menu toggles. An iframe-backed sibling opens directly against the source's inline end at the measured Gutenberg sidebar width (320px fallback), matching its top and bottom edges; the shell shifts or minimally narrows the source only when the pair needs room. Once Gutenberg is ready, detached CSS hides the companion's duplicate canvas and stretches its complementary area. The shell suppresses the companion's separate title bar and shadow, uses a row of Gutenberg-style sidebar icons to continue the source title-bar surface across the inspector, and leaves the sidebar as a flat white panel behind one divider. Gutenberg's native Settings and plugin-sidebar buttons keep controlling the independent in-editor panel, while the matching Sidecar icons control only the external copy. It looks and moves like one editor with an inspector extension, not a full-screen tile or a pair of competing windows.

This can be a compatibility multiplier for Jetpack, Yoast, Rank Math, ACF, and other surfaces registered as Gutenberg sidebars: the plugin loads normally in the companion, and its familiar toggle artwork is reused in the Sidecar toolbar whenever Gutenberg exposes it. Pressing a discovered Sidecar icon or choosing the area from the source launch menu switches the same companion through Gutenberg's complementary-area API. Plugin authors do not register a second OpenStation-specific UI or switching control; they continue using `PluginSidebar` and the complementary-area APIs. Compatibility still needs verification because the sibling is another editor document, not a view onto the source store.
...
After the companion's `os-ready`, it receives `os-editor-sidecar-set { active: true, detached: true, area }` and renders its own copy of that plugin surface. No command is sent to the source iframe, so its active Post, Block, or plugin panel remains visible and follows ordinary Gutenberg controls. Closing either the managed sibling or Gutenberg's area inside it clears only the Sidecar button and persistence. The companion is `ephemeral`, so only the source participates in session snapshots; `openstation.editorSidecar.activeWindows` is used to create a fresh sibling after a restored source becomes ready. See [`bridge-protocol.md`](./bridge-protocol.md#sidebar-window-state--os-editor-sidecar-) for the full exchange.
