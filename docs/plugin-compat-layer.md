...
## The editor side: managed Sidebar Window

The **Sidebar Window** is the broad compatibility path for plugins that already integrate with Gutenberg's Post / Block sidebar system. It is not a per-plugin shim: the source editor's outer title-bar button opens a launch chooser for Post, Block, and plugin areas, then parks the source's current complementary area while leaving it floating. Gutenberg's store exposes the active identifier but not an enumeration of every registered area, so OpenStation discovers plugin registrations from the same `aria-controls` identifiers used by Gutenberg's pinned and menu toggles. An iframe-backed sibling opens directly against the source's inline end at the measured Gutenberg sidebar width (320px fallback), aligned to the editor body below the source's normal title bar; the shell shifts or minimally narrows the source only when the pair needs room. Once Gutenberg is ready, detached CSS hides the companion's duplicate canvas and stretches its complementary area. The shell suppresses the companion's separate title bar and shadow, presents it as a flat white drawer with one divider, and exposes the discovered choices through a compact **Sidebar panel** selector row. While connected, Gutenberg's native Settings and plugin-sidebar buttons in the source editor are also routed to the companion, so the familiar plugin controls remain direct switching shortcuts. It looks and moves like one editor with an inspector extension, not a full-screen tile or a pair of competing windows.

This can be a compatibility multiplier for Jetpack, Yoast, Rank Math, ACF, and other surfaces registered as Gutenberg sidebars: the plugin loads normally in the companion, and selecting another discovered area from `<os-select class="os-editor-sidecar-panel-select" label="Sidebar panel">` or the plugin's existing Gutenberg button switches the same companion through Gutenberg's complementary-area API. Plugin authors do not register a second OpenStation-specific UI or switching control; they continue using `PluginSidebar` and the complementary-area APIs. Compatibility still needs verification because the sibling is another editor document, not a view onto the source store.

The architecture has two non-negotiable boundaries and one explicit tradeoff:

1. **No arbitrary React DOM adoption into another document.** Moving the source sidebar node to a shell-owned panel would detach it from assumptions made by its React root: delegated events, contexts, portals, stylesheets, owner-document queries, focus traps, and measurement APIs. A panel that looks intact can still be functionally broken.
2. **An iframe cannot paint into a sibling window.** A single Gutenberg document cannot occupy the source window while projecting one React subtree into another independently draggable, clipped, and stacked shell rectangle.
3. **A real sibling therefore means two Gutenberg instances.** The companion has a separate React/data registry, dirty flag, undo history, autosave timer, post-lock view, and save lifecycle for the same post. OpenStation transfers only the selected area id and lifecycle signals; it does not synchronize editor stores. A plugin panel that mutates `core/editor` can race or overwrite source changes, while a panel persisting independent REST-backed plugin state may be less coupled. Test the actual save path, not just first paint.

The source receives `os-editor-sidecar-source { parked: true }`, which remembers and closes the exact area there. After the companion's `os-ready`, it receives `os-editor-sidecar-set { active: true, detached: true, area }` and renders its own copy of that plugin surface. Closing either the managed sibling or Gutenberg's area inside it clears the source button and persistence, unparks the source, and restores the remembered area. The companion is `ephemeral`, so only the source participates in session snapshots; `openstation.editorSidecar.activeWindows` is used to create a fresh sibling after a restored source becomes ready. See [`bridge-protocol.md`](./bridge-protocol.md#sidebar-window-state--os-editor-sidecar-) for the full exchange.
...
6. **Does the plugin already register a Gutenberg sidebar that works in classic wp-admin?** → Verify it in Sidebar Window before writing a bespoke integration. The companion loads the plugin's normal Gutenberg surface, but it is a second editor document: test dirty state, autosave, locks, and final persistence against simultaneous edits in the source.
