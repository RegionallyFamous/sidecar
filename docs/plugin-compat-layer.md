...
The **Sidebar Window** is the broad compatibility path for plugins that already integrate with Gutenberg's Post / Block sidebar system. It is not a per-plugin shim: the source editor's outer title-bar button opens a chooser for Post, Block, and plugin areas, then parks the source's current complementary area while leaving it floating. Gutenberg's store exposes the active identifier but not an enumeration of every registered area, so the chooser discovers plugin registrations from the same `aria-controls` identifiers used by Gutenberg's pinned and menu toggles. An iframe-backed sibling opens directly against the source's inline end at the measured Gutenberg sidebar width (320px fallback), with the same top edge and height; the shell shifts or minimally narrows the source only when the pair needs room. The companion loads the same post URL with `openstation_sidebar_window=1`; once Gutenberg is ready, detached CSS hides its duplicate canvas and stretches its own complementary area beneath standard OpenStation window chrome. This is a narrow attached pair, not a full-screen left/right tile.

This can be a compatibility multiplier for Jetpack, Yoast, Rank Math, ACF, and other surfaces registered as Gutenberg sidebars: the plugin loads normally in the companion, and selecting another discovered area switches the same companion through Gutenberg's complementary-area API. Plugin authors do not register a second OpenStation-specific UI; they continue using `PluginSidebar` and the complementary-area APIs. Compatibility still needs verification because the sibling is another editor document, not a view onto the source store.
