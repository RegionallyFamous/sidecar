...
The **Sidebar Window** is the broad compatibility path for plugins that already integrate with Gutenberg's Post / Block sidebar system. It is not a per-plugin shim: the source editor's outer title-bar button opens a launch chooser for Post, Block, and plugin areas, then parks the source's current complementary area while leaving it floating. Gutenberg's store exposes the active identifier but not an enumeration of every registered area, so OpenStation discovers plugin registrations from the same `aria-controls` identifiers used by Gutenberg's pinned and menu toggles. An iframe-backed sibling opens directly against the source's inline end at the measured Gutenberg sidebar width (320px fallback), matching its top and bottom edges; the shell shifts or minimally narrows the source only when the pair needs room. Once Gutenberg is ready, detached CSS hides the companion's duplicate canvas and stretches its complementary area. The shell suppresses the companion's separate title bar and shadow, uses the selector row to continue the source title-bar surface across the inspector, and leaves the sidebar as a flat white panel behind one divider. While connected, Gutenberg's native Settings and plugin-sidebar buttons in the source editor are also routed to the companion, so the familiar plugin controls remain direct switching shortcuts. It looks and moves like one editor with an inspector extension, not a full-screen tile or a pair of competing windows.
