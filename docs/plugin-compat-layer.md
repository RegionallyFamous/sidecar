...
The **Sidebar Window** is the broad compatibility path for plugins that already integrate with Gutenberg's Post / Block sidebar system. It is not a per-plugin shim: the source editor's outer title-bar button captures and parks its current complementary area while leaving the source floating. An iframe-backed sibling opens directly against the source's inline end at the measured Gutenberg sidebar width (320px fallback), with the same top edge and height; the shell shifts or minimally narrows the source only when the pair needs room. The companion loads the same post URL with `openstation_sidebar_window=1`; once Gutenberg is ready, detached CSS hides its duplicate canvas and stretches its own complementary area beneath standard OpenStation window chrome. This is a narrow attached pair, not a full-screen left/right tile.

This can be a compatibility multiplier for Yoast, Rank Math, ACF, and other surfaces registered as Gutenberg sidebars: the plugin loads normally in the companion and Gutenberg still owns Post, Block, and plugin-area selection. Plugin authors do not register a second OpenStation-specific UI; they continue using `PluginSidebar` and the complementary-area APIs. Compatibility still needs verification because the sibling is another editor document, not a view onto the source store.
