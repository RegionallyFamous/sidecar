...
The **Sidebar Window** is the broad compatibility path for plugins that already integrate with Gutenberg's Post / Block sidebar system. It is not a per-plugin shim: the source editor's outer title-bar button opens a launch chooser for Post, Block, and plugin areas, then parks the source's current complementary area while leaving it floating. Gutenberg's store exposes the active identifier but not an enumeration of every registered area, so OpenStation discovers plugin registrations from the same `aria-controls` identifiers used by Gutenberg's pinned and menu toggles. An iframe-backed sibling opens directly against the source's inline end at the measured Gutenberg sidebar width (320px fallback), with the same top edge and height; the shell shifts or minimally narrows the source only when the pair needs room. The companion loads the same post URL with `openstation_sidebar_window=1`; once Gutenberg is ready, detached CSS hides its duplicate canvas and stretches its own complementary area beneath standard OpenStation window chrome. An always-visible **Sidebar panel** selector in the companion's after-title-bar slot exposes the same discovered choices without sending the user back to the source window. While connected, Gutenberg's native Settings and plugin-sidebar buttons in the source editor are also routed to the companion, so the familiar plugin controls remain direct switching shortcuts. This is a narrow attached pair, not a full-screen left/right tile.
