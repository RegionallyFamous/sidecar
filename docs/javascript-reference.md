...
Fires when the user picks a new top-level desktop layout in OS Settings → Appearance. The shell tears down and rebuilds the dock(s) before the event fires; plugins that cached `wp.os.dock` should re-fetch from the event detail (or read `wp.os.dock` again — it's mutated in place). The shell root reflects the new value in `data-os-layout` attribute by the time this fires, so CSS selectors keyed on it will already match.
...
Fires on every phase transition of an OS Settings save — both the built-in panel's edits and programmatic patches via [`wp.os.updateOsSettings()`](#updateossettings-patch-opts---stable).
...
> **Layout switching note** — the underlying instance is replaced when the user picks a new layout in OS Settings → Appearance. `wp.os.dock` is mutated in place so a fresh property read returns the current dock; plugins that **cache** the reference earlier should listen for `os-layout-changed` and refresh.
...
Currently-active top-level layout. One of `'classic' | 'unified' | 'spatial'`. Mirrors the user's OS Settings → Appearance pick and the `data-os-layout` attribute on the shell root.
...
`enable()` / `disable()` / `toggle()` write the per-user OS setting `mioEnabled` exactly as the dock tile does. **The user's look is per-user too** — it rides the same OS Settings blob as `mioStyle`, so a Mio built on a laptop is waiting on the phone. Only the resting position is browser-local (`localStorage`, `os-mio-position`): where Mio sits is a fact about one screen, how it looks is a fact about the person.
...
### The "Sidebar Window" title-bar button — Experimental

Gutenberg post-editor windows show a **Sidebar Window** button (columns icon, right side of the outer OpenStation title bar, immediately before Preview; registered as `desktop-mode/editor-sidecar`). It appears only after the iframe has booted both the `core/editor` store and a complementary-area store (`core/interface` or the legacy `core/edit-post` store), so Classic Editor and unrelated admin screens never receive a dead control. Clicking it opens an `os-context-menu` chooser containing Post settings, Block settings, the active area, and plugin sidebars discovered from Gutenberg's own complementary-area toggle controls. This includes ordinary `PluginSidebar` registrations such as Jetpack when their toggle is present.

Choosing an area creates a connected editor without turning the desktop into a half-screen split. After the selected complementary-area id and sidebar width are captured, the source editor keeps its active area, iframe, and floating state unchanged. An iframe-backed companion named `${sourceWindowId}--sidebar-window` opens on the same virtual desktop at the measured sidebar width (320px fallback), directly adjacent at the source's inline end with the same top and bottom edges. The shell shifts or minimally narrows the source only when the connected surface would otherwise cross the desktop edge. It loads the source's current post URL with `openstation_sidebar_window=1` and carries `ephemeral: true`. Visually, the source keeps its ordinary OpenStation window while the companion suppresses its own title bar, controls, and shadow. Its icon row continues the source title-bar colors and texture across the inspector without a second outline or exposed gap; the white sidebar body begins beneath that single visual title bar. Only the source receives the **Sidebar Window** launch chooser, so companions cannot recursively detach another sidebar. The inspector mounts `.os-editor-sidecar-panel-buttons[role="toolbar"]`, an accessible `<os-window-button data-sidebar-area="…">` for every Post, Block, and discovered plugin area, plus a close control inside `.os-window__slot--after-titlebar`. It imports the corresponding SVG, image, or Dashicon from Gutenberg's own toggle when available, supplies WordPress fallbacks for core choices, and marks the current Sidecar area with `aria-pressed="true"` and `active`. The source chooser remains available and also provides **Close Sidebar Window** while active.

The source editor's native Gutenberg sidebar remains fully independent. While Sidebar Window is connected, clicking Settings or a Jetpack/other plugin sidebar button in Gutenberg's own top bar changes the in-editor panel only. The matching icon in the companion toolbar changes the external panel only. Both can therefore stay open at once and can show different complementary areas without either control stealing the other surface.

The shell waits for the companion iframe's `os-ready` handshake before sending `{ active: true, detached: true, area }`. The companion opens that area through `core/interface` or `core/edit-post`; detached CSS hides its duplicate Gutenberg header, post canvas, and footer and stretches its complementary area across the iframe. The continuous title-bar surface, familiar icon controls, pane close control, and quiet internal divider make the companion read as an inspector extension rather than another window. Selecting a panel from the companion toolbar or source launch chooser sends another `os-editor-sidecar-set` to the same companion iframe; neither path opens another companion or modifies the source sidebar. Resize, maximize, fullscreen, and detach controls are suppressed so it cannot become a half/full-screen tile. No faux title bar or sidebar DOM is injected into the source editor.
...
Closing the companion, choosing **Close Sidebar Window** from the source chooser, or closing Gutenberg's complementary area inside the companion clears the pressed/persisted Sidecar state without changing the source sidebar. No sidebar restoration step is needed because the source was never parked. Closing the source destroys its companion.
...
Open (or focus, if already open) the shell's OS Settings window. Routes through the same `windowManager.open()` call the dock's OS Settings tile uses, so a window opened via `wp.os.openOsSettings()` is indistinguishable from one opened by clicking the dock tile — same id, same render callback, same dimensions, same focus / minimize behaviour.

```js
wp.os.openOsSettings();
```

Pass `{ tabId }` to land directly on a specific settings tab. The built-in tab ids are `'appearance'`, `'ai'`, `'apps-icons'`, `'features'`, `'effects'`, `'help'`, and `'about'`; a tab registered via `registerSettingsTab()` is addressable by its own id. (`'extended'` is accepted as a legacy alias for `'features'` — the Extended Options tab merged into the Features tab.) The tab is selected before the window opens, and if OS Settings is already open the live tab strip switches in place:
...
The motivating use case: a custom dock rail renderer in **Classic** layout doesn't see the OS Settings tile (it lives on the side rail with the core menus, not the primary rail the custom renderer owns). Opening OS Settings from inside the renderer used to require DOM-scraping `[data-system-id="os-settings"]` and clicking it; this method is the documented portable path.
...
Reports the effective state of the managed companion after an [`os-editor-sidecar-set`](#os-editor-sidecar-set--experimental) request or after Gutenberg removes its complementary area. The parent accepts this only from the expected `${sourceWindowId}--sidebar-window` iframe. While active, `area` keeps the companion's pressed sidebar icon synchronized with Gutenberg's current panel. `active: false` or `available: false` closes the sibling and clears the persisted source id without changing the source iframe. `width` is the iframe bridge's legacy inner-sidebar diagnostic, not the outer OpenStation window width.

```typescript
{ type: 'os-editor-sidecar-state'; active: boolean; available: boolean; width: number; area: string | null }
```
...
Activates a complementary area inside a Sidebar Window iframe, then answers with [`os-editor-sidecar-state`](#os-editor-sidecar-state--experimental). The managed sibling receives this only after `IFRAME_READY`, with `detached: true` and the area id captured from the source. Detached mode hides that iframe's duplicate editor canvas so its own Gutenberg sidebar fills the outer OpenStation window. `active: false` removes the mode and closes the iframe's active area; pages without the required stores and DOM answer with `active: false, available: false`.
...
The shell ships a registry-driven wallpaper picker: every entry in the registry becomes a swatch in the OS Settings panel, and the WallpaperLayer resolves whichever is currently selected onto the desktop. Plugins register their own via `wp.os.registerWallpaper()` (or the `os.wallpapers` filter).
...
type WallpaperDef =
    | {
          ...
          description?: string;       // Plain text, shown in OS Settings when selected
...
Wallpapers with real tunables (particle counts, palettes, physics) can ship a `renderConfig` callback. When the wallpaper is the active selection in OS Settings, a **"Wallpaper settings"** button appears below the picker grid; clicking it opens a `<os-modal>` whose body is handed to your callback. Wallpapers without `renderConfig` show no button — the surface is invisible unless you opt in.
