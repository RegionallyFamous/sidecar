...
Choosing an area creates a compound editor without turning the desktop into a half-screen split. After the selected complementary-area id and sidebar width are captured, the source editor parks its active area but keeps its iframe and floating state. An iframe-backed companion named `${sourceWindowId}--sidebar-window` opens on the same virtual desktop at the measured sidebar width (320px fallback), aligned with the source's top edge, equal to its height, and directly adjacent at its inline end. The shell shifts or minimally narrows the source only when the compound surface would otherwise cross the desktop edge. It loads the source's current post URL with `openstation_sidebar_window=1` and carries `ephemeral: true`. Visually, the companion's own title bar, border, and shadow are suppressed: the source draws one outer frame around both regions, and focus on either region keeps that shared frame active. Only the source receives the **Sidebar Window** launch chooser, so companions cannot recursively detach another sidebar. The attached inspector mounts a compact, always-visible `<os-select class="os-editor-sidecar-panel-select" label="Sidebar panel" compact>` plus a close control inside `.os-window__slot--after-titlebar`. It lists Post, Block, and the discovered plugin areas, reflects the current area, and switches the existing companion through the same activation message. The source chooser remains available and also provides **Close Sidebar Window** while active.
...
The shell waits for the companion iframe's `os-ready` handshake before sending `{ active: true, detached: true, area }`. The companion opens that area through `core/interface` or `core/edit-post`; detached CSS hides its duplicate Gutenberg header, post canvas, and footer and stretches its complementary area across the iframe. The shared outer frame, compact selector toolbar, pane close control, and quiet internal divider make the companion read as an extension of the editor rather than another window. Selecting a panel from the companion, the source launch chooser, or Gutenberg's routed source controls sends another `os-editor-sidecar-set` to the same iframe; none of these paths opens another companion. Resize, maximize, fullscreen, and detach controls are suppressed so it cannot become a half/full-screen tile. No faux title bar or sidebar DOM is injected into the source editor.
