...
Reports the effective state of the managed companion after an [`os-editor-sidecar-set`](#os-editor-sidecar-set--experimental) request or after Gutenberg removes its complementary area. The parent accepts this only from the expected `${sourceWindowId}--sidebar-window` iframe. While active, `area` keeps the companion's visible selector synchronized with Gutenberg's current panel. `active: false` or `available: false` closes the sibling and clears the persisted source id without changing the source iframe. `width` is the iframe bridge's legacy inner-sidebar diagnostic, not the outer OpenStation window width.
