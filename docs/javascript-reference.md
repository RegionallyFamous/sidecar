...
Choosing an area creates a connected editor without turning the desktop into a half-screen split. After the selected complementary-area id and sidebar width are captured, the source editor parks its active area but keeps its iframe and floating state. An iframe-backed companion named `${sourceWindowId}--sidebar-window` opens on the same virtual desktop at the measured sidebar width (320px fallback), directly adjacent at the source's inline end with the same top and bottom edges. The shell shifts or minimally narrows the source only when the connected surface would otherwise cross the desktop edge. It loads the source's current post URL with `openstation_sidebar_window=1` and carries `ephemeral: true`. Visually, the source keeps its ordinary OpenStation window while the companion suppresses its own title bar, controls, and shadow. Its selector row continues the source title-bar colors and texture across the inspector without a second outline or exposed gap; the white sidebar body begins beneath that single visual title bar. Only the source receives the **Sidebar Window** launch chooser, so companions cannot recursively detach another sidebar. The inspector mounts a compact, always-visible `<os-select class="os-editor-sidecar-panel-select" label="Sidebar panel" compact>` plus a close control inside `.os-window__slot--after-titlebar`. It lists Post, Block, and the discovered plugin areas, reflects the current area, and switches the existing companion through the same activation message. The source chooser remains available and also provides **Close Sidebar Window** while active.
...
The shell waits for the companion iframe's `os-ready` handshake before sending `{ active: true, detached: true, area }`. The companion opens that area through `core/interface` or `core/edit-post`; detached CSS hides its duplicate Gutenberg header, post canvas, and footer and stretches its complementary area across the iframe. The continuous title-bar surface, transparent selector treatment, pane close control, and quiet internal divider make the companion read as an inspector extension rather than another window. Selecting a panel from the companion, the source launch chooser, or Gutenberg's routed source controls sends another `os-editor-sidecar-set` to the same iframe; none of these paths opens another companion. Resize, maximize, fullscreen, and detach controls are suppressed so it cannot become a half/full-screen tile. No faux title bar or sidebar DOM is injected into the source editor.
...
Closing the companion, choosing **Close Sidebar Window** from the source chooser, or closing Gutenberg's complementary area inside the companion clears the pressed/persisted state and unparks the source, restoring the exact area that was active before detaching. No tiled state needs restoring because the source stayed floating. Closing the source destroys its companion.
...
Reports the effective state of the managed companion after an [`os-editor-sidecar-set`](#os-editor-sidecar-set--experimental) request or after Gutenberg removes its complementary area. The parent accepts this only from the expected `${sourceWindowId}--sidebar-window` iframe. While active, `area` keeps the companion's visible selector synchronized with Gutenberg's current panel. `active: false` or `available: false` closes the sibling, unparks the source, and clears the persisted source id. `width` is the iframe bridge's legacy inner-sidebar diagnostic, not the outer OpenStation window width.
...
#### `os-editor-sidecar-source` — Experimental

Parks or restores the source editor's complementary area while its managed sibling is open. On the first `parked: true`, the iframe remembers the exact active area and closes it; duplicate park messages are idempotent. `parked: false` reopens the remembered area so Post, Block, and plugin panels return to the source after the sibling closes.
