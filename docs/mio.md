...
Mio Companion is the built-in `openstation/mio` widget. It presents one code-native, egg-shaped pixel virtual-pet shell with a recessed Imagegen habitat, a thinner outlined Mio based on the official rounded blob silhouette, a greeting target, and three raised care controls. The unlettered casing is authored on a 108 × 150 logical grid and displayed at an exact 2× scale: edges stay crisp, but the silhouette, material ramps, and Miomesh colour bands have the finer cadence of 16-bit-era pixel art. The translated Mio title remains in visually hidden markup as the section's accessible name. The LCD is physically inset inside the shell shoulders, with a dark bezel and clipped host overflow as independent containment guards. It exposes no meters, scores, streaks, or failure states. The LCD message lane is empty and visually absent at rest, leaving Mio centered in the full habitat frame. Greeting Mio or choosing a care action steps Mio upward and reveals its response for 2.6 seconds; another action restarts that window. The persistent empty node remains a polite live region, all actions are native buttons, and visible plus accessible copy runs through the `desktop-mode` text domain before being inserted as text. The host card surface is transparent so the device reads as one object while the standard draggable chrome remains available. It supports a 240 × 360 minimum, 270 × 390 default, and 340 × 460 maximum floating size.
