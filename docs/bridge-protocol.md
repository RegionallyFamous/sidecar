...
Sidebar Window is a real shell-managed sibling attached to the source editor's inline end, but the shell presents it as an inspector drawer. The source keeps its original iframe, editable post canvas, floating state, and normal OpenStation window frame. A transient iframe-backed companion named `${sourceWindowId}--sidebar-window` opens at the source Gutenberg sidebar's measured width (320px fallback), aligned to the editor body below the source title bar and ending at the same bottom edge. The shell shifts or minimally narrows the source only when necessary to keep the whole connected surface on the desktop. It loads the source's current post URL with `openstation_sidebar_window=1`. After that companion announces `os-ready`, the shell selects the same complementary-area id and applies detached presentation CSS that hides the duplicate Gutenberg header, canvas, and footer. The companion's own OpenStation title bar and shadow are suppressed; a flat white selector row and quiet divider identify the attached inspector pane. Neither region is expanded into a full-screen split.
...
4. The shell waits for the companion's `IFRAME_READY` / `os-ready` signal, then sends `os-editor-sidecar-set { active: true, detached: true, area }`. The iframe uses `core/interface` (or legacy `core/edit-post`) to open that area. `html` and `body` gain `os-editor-sidecar-detached`; CSS hides the duplicate editor canvas and lets `.interface-interface-skeleton__sidebar` occupy the whole iframe. The source keeps its normal window frame while the shell suppresses the companion's title bar and shadow. A compact **Sidebar panel** selector plus close button occupies a flat 40px row at the top of the drawer. Resize/maximize/fullscreen controls stay suppressed so the pane keeps the source-sidebar geometry, and focus on either region keeps the source title bar visually active.
...
- `os-window--editor-sidecar-pair-focused` keeps the source frame and title bar visually focused while either underlying shell window owns input focus.
