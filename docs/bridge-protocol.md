...
Sidebar Window is an attached-window presentation inside the existing Gutenberg iframe. The shell owns the outer title-bar affordance and restored-window policy; the iframe adds the visible attached title bar and close control, gap, border, resizer, and width around Gutenberg's live complementary area. The chrome makes the sidebar read as its own attached window, but there is still one `core/editor` store, one block-editor store, one React tree, one undo history, and one autosave lifecycle.

| Type | Direction | Carries | Purpose |
|---|---|---|---|
| `os-editor-sidecar-set` | parent → iframe | `{ active: boolean }` | Create or remove the attached Sidebar Window chrome around Gutenberg's existing complementary area. Idempotent; pages without the required editor stores answer with unavailable state. |
| `os-editor-sidecar-state` | iframe → parent | `{ active: boolean, available: boolean, width: number }` | Report the effective attached-window state after a set request or close action. The shell uses `active` / `available` to synchronize the outer title-bar button and persisted active-window ids; `width` exposes the current clamped width for diagnostics. |
...
2. Clicking the button updates the shell's active-editor set and posts `os-editor-sidecar-set`. A narrow editor is maximized once when the desktop is wide enough for the canvas and attached window.
3. Inside that same iframe, the handler frames the complementary area Gutenberg already rendered with attached window chrome: its own title bar and close control, a border and visual gap from the canvas, plus a leading-edge resizer. The plugin-owned React sidebar stays in its original document and data registry.
4. The resizer changes the attached window width and persists it as `openstation.editorSidecar.width` in local storage. Width starts at 320px, normally clamps to 280–520px, and is further capped to leave at least 320px of editor canvas (with a 240px floor when the whole iframe is narrow). The separator supports pointer dragging, Arrow / Shift+Arrow, Home / End, and a double-click reset.
5. The iframe posts `os-editor-sidecar-state`; using the attached title bar's close control clears the shell's active state and repaints the outer title-bar button.
6. A session-restored editor gets a new iframe document. The shell waits for that document's `os-ready` message, re-runs the Gutenberg capability probe, and only then rebuilds the attached chrome for persisted active state. The same path handles in-place editor navigations and reloads without racing the new listener.

The protocol intentionally never transports block content, editor state, or rendered sidebar markup. Starting a second Gutenberg instance would create two writers for dirty state, post locks, autosaves, and saves. Moving arbitrary React nodes into another document is not a safe shortcut either: plugin sidebars depend on their original React root, delegated event system, context providers, portals, styles, and document-scoped focus behavior. The injected attached-window chrome is ordinary UI inside the existing iframe, not a second browsing context; keeping the real complementary area in its owning document lets normal `PluginSidebar` integrations work without OpenStation-specific adapters.
...
- `.os-editor-sidecar-window-chrome` marks the visible attached title bar and close-control chrome inside the editor iframe.
- `.os-editor-sidecar-resizer` is the injected `role="separator"` handle on the attached window's leading edge; `body.os-editor-sidecar-resizing` marks an active pointer drag.
