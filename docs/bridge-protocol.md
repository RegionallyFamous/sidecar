...
Sidebar Window is a real shell-managed sibling of the source editor. The source keeps its original iframe and editable post canvas, snapped left; a transient iframe-backed companion named `${sourceWindowId}--sidebar-window` snaps right and loads the source's current post URL with `openstation_sidebar_window=1`. After that companion announces `os-ready`, the shell selects the same complementary-area id and applies detached presentation CSS that hides the companion's duplicate header, canvas, and footer. The remaining Gutenberg sidebar fills the companion beneath ordinary OpenStation window chrome.
...
2. Clicking it captures the source's active area, persists the source window id, sends `os-editor-sidecar-source { parked: true }`, snaps the source left, and opens `${sourceWindowId}--sidebar-window` snapped right on the same virtual desktop. The source iframe is not replaced or reparented.
...
4. The shell waits for the companion's `IFRAME_READY` / `os-ready` signal, then sends `os-editor-sidecar-set { active: true, detached: true, area }`. The iframe uses `core/interface` (or legacy `core/edit-post`) to open that area. `html` and `body` gain `os-editor-sidecar-detached`; CSS hides the duplicate editor canvas and lets `.interface-interface-skeleton__sidebar` occupy the whole iframe. OpenStation's outer window supplies the visible title bar, close button, border, drag surface, and resize handles; the old injected faux title bar is gone.
5. Closing the sibling, toggling the source button off, or closing Gutenberg's sidebar inside the sibling clears the active id, sends `os-editor-sidecar-source { parked: false }`, restores the exact source area, and repaints the source button. During the same page lifetime, OpenStation also restores the source's pre-sidecar layout when possible. Closing the source destroys its companion immediately.
6. The companion has `ephemeral: true`, so it is excluded from session snapshots and never restored independently. The parent stores at most 64 active source ids in `openstation.editorSidecar.activeWindows`; when a source editor itself is session-restored, the shell waits for that source iframe to become Gutenberg-ready and creates a fresh ephemeral companion. The restored source window's own session geometry remains authoritative.
...
- `--os-editor-sidecar-width` and `.os-editor-sidecar-resizer` remain part of the bridge's legacy non-detached width contract. The inner resizer is hidden in detached companions because the outer OpenStation window is the visible resize surface.
