...
Sidebar Window is a real shell-managed sibling attached to the source editor's inline end, but the shell presents it as an inspector extension. The source keeps its original iframe, editable post canvas, active sidebar, floating state, and normal OpenStation window. A transient iframe-backed companion named `${sourceWindowId}--sidebar-window` opens at the source Gutenberg sidebar's measured width (320px fallback), matching the source's top and bottom edges. The shell shifts or minimally narrows the source only when necessary to keep the whole connected surface on the desktop. It loads the source's current post URL with `openstation_sidebar_window=1`. After that companion announces `os-ready`, the shell selects the requested complementary-area id and applies detached presentation CSS that hides the duplicate Gutenberg header, canvas, and footer. The companion's own OpenStation title bar and shadow are suppressed; its compact sidebar-icon row continues the source title-bar surface across the inspector with no second outline or exposed background gap. Neither region is expanded into a full-screen split, and both sidebar instances remain usable at once.
...
| `os-editor-sidecar-state` | companion iframe → parent | `{ active: boolean, available: boolean, width: number, area: string \| null }` | Report whether the companion exposed a compatible complementary area. While active, `area` keeps the companion's pressed sidebar icon synchronized with Gutenberg's current panel. `active: false` or `available: false` makes the shell close the sibling, clear persistence, and repaint the source title-bar button without changing the source iframe. `width` is the bridge's legacy inner-sidebar width diagnostic, not the outer OpenStation window width. |
...
2. Clicking it opens a shell `os-context-menu` containing Post settings, Block settings, the active area, and plugin areas discovered from Gutenberg's pinned/menu toggle `aria-controls` identifiers. Choosing an area captures that identifier, measures the source sidebar width, and persists the source window id. The source stays in normal floating state with its complementary area open and untouched. The shell opens `${sourceWindowId}--sidebar-window` on the same virtual desktop at that measured width (320px fallback), directly against the source's inline end with matching top and bottom edges. If the pair would cross a desktop edge, the source is shifted or minimally narrowed to fit. The source iframe is not replaced, reparented, maximized, snapped into a half-screen tile, or sent any sidebar command.
...
4. The shell waits for the companion's `IFRAME_READY` / `os-ready` signal, then sends `os-editor-sidecar-set { active: true, detached: true, area }`. The iframe uses `core/interface` (or legacy `core/edit-post`) to open that area. `html` and `body` gain `os-editor-sidecar-detached`; CSS hides the duplicate editor canvas and lets `.interface-interface-skeleton__sidebar` occupy the whole iframe. The shell suppresses the companion's own title bar and shadow. A compact row of sidebar toggle icons plus a close button occupies the top 40px and inherits the source title-bar surface, making one continuous header across editor and inspector. Icons are imported from Gutenberg's same-origin toggle controls when available, preserving familiar plugin artwork; Post and Block receive WordPress glyph fallbacks. Resize/maximize/fullscreen controls stay suppressed so the pane keeps the source-sidebar geometry, and focus on either region keeps that header visually active.
5. Pressing a Post, Block, or discovered plugin icon in the companion sends a fresh active `os-editor-sidecar-set` to the existing companion; the iframe switches through its own complementary-area store without opening another window. The pressed icon tracks the companion's active area. The source title-bar chooser remains available and can also switch the active companion. Independently, Gutenberg's own Settings, Jetpack, and other plugin-sidebar buttons continue switching the sidebar inside the source editor. This is the defining two-place behavior: source and Sidecar can show different panels simultaneously. Choosing **Close Sidebar Window** from the source title-bar chooser, closing the sibling, or closing Gutenberg's sidebar inside the sibling clears the Sidecar active id and repaints the source button without changing the source sidebar. Closing the source destroys its companion immediately.
...
- `.os-window__slot--after-titlebar` is the attached inspector toolbar. It hosts `.os-editor-sidecar-panel-buttons[role="toolbar"]`, one accessible `<os-window-button data-sidebar-area="…">` per discovered complementary area, and the pane close control. The active button carries `aria-pressed="true"` and `active`.
...
- `[aria-controls]` values matching Gutenberg's complementary-area identifier shape expose choices and reusable icon artwork for the source launch chooser and companion toolbar. The `core/interface` store supplies the active id but has no public registered-area enumeration selector. Once open, the source and companion stores are intentionally independent.
