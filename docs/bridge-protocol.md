...
Sidebar Window is a real shell-managed sibling attached to the source editor's inline end, but the shell presents it as an inspector extension. The source keeps its original iframe, editable post canvas, floating state, and normal OpenStation window. A transient iframe-backed companion named `${sourceWindowId}--sidebar-window` opens at the source Gutenberg sidebar's measured width (320px fallback), matching the source's top and bottom edges. The shell shifts or minimally narrows the source only when necessary to keep the whole connected surface on the desktop. It loads the source's current post URL with `openstation_sidebar_window=1`. After that companion announces `os-ready`, the shell selects the same complementary-area id and applies detached presentation CSS that hides the duplicate Gutenberg header, canvas, and footer. The companion's own OpenStation title bar and shadow are suppressed; its compact selector row continues the source title-bar surface across the inspector with no second outline or exposed background gap. Neither region is expanded into a full-screen split.
...
4. The shell waits for the companion's `IFRAME_READY` / `os-ready` signal, then sends `os-editor-sidecar-set { active: true, detached: true, area }`. The iframe uses `core/interface` (or legacy `core/edit-post`) to open that area. `html` and `body` gain `os-editor-sidecar-detached`; CSS hides the duplicate editor canvas and lets `.interface-interface-skeleton__sidebar` occupy the whole iframe. The shell suppresses the companion's own title bar and shadow. A compact **Sidebar panel** selector plus close button occupies the top 40px and inherits the source title-bar surface, making one continuous header across editor and inspector. Resize/maximize/fullscreen controls stay suppressed so the pane keeps the source-sidebar geometry, and focus on either region keeps that header visually active.
