...
5. Closing the sibling, toggling the source button off, or closing Gutenberg's sidebar inside the sibling clears the active id, sends `os-editor-sidecar-source { parked: false }`, restores the exact source area, and repaints the source button. The source window needs no size/state restoration because opening the attached sibling did not retile it. Closing the source destroys its companion immediately.
