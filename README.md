...
- **Gutenberg Sidebar Window**
  Gutenberg editor windows gain a title-bar toggle that opens the current Post, Block, or plugin sidebar in a real sibling OpenStation window. The source editor snaps left and parks its sidebar; an ephemeral iframe-backed companion snaps right, loads the same post URL, and hides its duplicate canvas so only Gutenberg's complementary area remains beneath ordinary OpenStation title-bar, close, drag, and resize chrome. A true sibling requires a second Gutenberg document, with a separate data registry, undo history, dirty state, and autosave lifecycle; OpenStation does not synchronize those two editor instances.
