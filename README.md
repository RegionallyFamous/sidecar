...
- **Gutenberg Sidebar Window**
  Gutenberg editor windows gain a title-bar toggle that presents the current Post, Block, or plugin sidebar as a visibly attached window beside the post canvas. The attached window has its own title bar and close control, a border and gap from the canvas, and a resizable persisted width — all inside the original editor iframe, without starting a second Gutenberg instance or competing autosave session.
